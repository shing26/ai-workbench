...
| ID | 任务 | 验收标准 |
|---|---|---|
| T1 | 工程基线：文档树、Git 分支、cargo fmt/clippy、npm build | 新分支 `feature/sprint-1-app-shell` 可提交，提交信息符合 Conventional Commits；质量门：`cargo fmt --check`、`cargo clippy --lib -D warnings`、`npm run build` |
...
- 云同步、登录、移动端。
- Monetization 收入自动聚合。
- ESLint/Prettier 与 husky/lint-staged 自动化接入。

## DoD 检查单

- [x] 界面完整性：5 个 Icon 平滑切换 5 个主视图。
- [x] 视觉对齐：卡片符合 `#18181C` + `border-white/10` + `rounded-2xl`。
- [x] 数据贯通：SQLite 初始化并读取示例数据，刷新后持久化。
- [x] 环境隔离：生产/开发遮罩隔离，无 Dev Issues 浮标。
- [x] build、lint、测试全绿，PR 已合并到 develop。
