...
export default function TopBar() {
  ...
  return (
    <header className="flex items-center h-[var(--topbar-height)] px-4 shrink-0 gap-3" style={{ background: "var(--color-bg-primary)", boxShadow: "inset 0 -1px 0 rgba(255,255,255,0.025)" }}>
      <h1 className="text-[12px] font-semibold tracking-tight select-none" style={{ color: "var(--color-text-primary)" }}>
        Workbench
      </h1>
      ...
      <div className="flex items-center gap-2 text-[10px]" style={{ color: "var(--color-text-muted)" }}>
        <div className="w-14 h-1 rounded-full overflow-hidden" style={{ background: "var(--color-surface-hover)" }}>
        ...
        <span className="tabular-nums">${budgetUsed.toFixed(0)}/{budgetLimit}</span>
      </div>
          ...
          {bellOpen && (
            ...
            >
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--color-border)]">
                <span className="text-[11px] font-medium text-[var(--color-text-primary)]">Notifications</span>
              </div>
              <div className="overflow-y-auto max-h-[280px] py-1">
                {urgentEvents.length === 0 ? (
                  <div className="px-4 py-8 text-center text-[11px] text-[var(--color-text-muted)]">No urgent notifications</div>
                ) : (
                  urgentEvents.map((evt) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="text-[11px] font-medium text-[var(--color-text-primary)] truncate">{evt.title}</div>
                        <div className="text-[10px] text-[var(--color-text-muted)] truncate mt-0.5">{evt.body}</div>
                      </div>
                      <span className="text-[9px] text-[var(--color-text-muted)] shrink-0 mt-0.5 tabular-nums">{relativeTime(evt.timestamp)}</span>
                    </button>
                  ))
                )}
              </div>
