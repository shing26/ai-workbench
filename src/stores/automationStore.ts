...
interface AutomationState {
  ...
  addTask: (name: string, cfg: Omit<TaskConfig, "id"|"name"|"status"|"lastRun"|"nextRun"|"createdAt">) => string;
...
let counter = 0;
function genId() { return `auto-${Date.now()}-${++counter}`; }
...
export function parseAutomationIntent(input: string): ParsedAutomation | null {
  ...
  else if (/min|分/.test(lower)) { const m = lower.match(/(\d+)\s*(?:min|分)/); schedule = m ? parseInt(m[1]) * 60 * 1000 : 5 * 60 * 1000; }
...
export const useAutomationStore = create<AutomationState>((set, get) => ({
  ...
  addTask: (name, cfg) => {
    const id = genId();
    const now = Date.now();
    const task: TaskConfig = { ...cfg, id, name, status: "running", lastRun: null, nextRun: now + cfg.schedule, createdAt: now };
  ...
  removeTask: (id) => set((s) => ({
    ...
    alertCount: s.logs.filter((l) => l.taskId !== id).filter((l) => l.level === "error" || l.level === "warning").length,
    selectedTaskId: s.selectedTaskId === id ? null : s.selectedTaskId,
  })),

  pauseTask: (id) => set((s) => ({ tasks: s.tasks.map((t) => t.id === id ? { ...t, status: "paused" as const } : t) })),
  resumeTask: (id) => set((s) => ({ tasks: s.tasks.map((t) => t.id === id ? { ...t, status: "running" as const } : t) })),

  setTaskStatus: (id, status) => set((s) => ({ tasks: s.tasks.map((t) => t.id === id ? { ...t, status } : t) })),

  updateLastRun: (id) => {
    const now = Date.now();
    set((s) => ({ tasks: s.tasks.map((t) => t.id === id ? { ...t, lastRun: now, nextRun: now + t.schedule } : t) }));
  },
