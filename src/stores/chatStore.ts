    ...
    sendChat(get().selectedModel, JSON.stringify(messages), get().ollamaModel)
      .then((response) => {
        set((s) => ({
          threads: s.threads.map((t) =>
            t.id === activeThreadId
              ? { ...t, messages: [...t.messages, { id: genId(), role: "assistant", content: response, timestamp: Date.now() }] }
              : t
          ),
      ...
      .catch((err) => {
        set((s) => ({
          threads: s.threads.map((t) =>
            t.id === activeThreadId
              ? { ...t, messages: [...t.messages, { id: genId(), role: "assistant", content: `请求失败: ${err}`, timestamp: Date.now() }] }
              : t
          ),
          isResponding: false,
        }));
