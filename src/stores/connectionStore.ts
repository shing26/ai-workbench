...
interface ConnectionState {
  ...
  addEvent: (evt: Omit<ConnectionEvent, "id" | "timestamp" | "level"> & { level?: SignalLevel }) => void;
  clearBadge: () => void;
  filteredEvents: (filters: { level?: SignalLevel | "all"; source?: SignalSource | "all"; search?: string }) => ConnectionEvent[];
}

let counter = 0;
function genId() { return `conn-${Date.now()}-${++counter}`; }

const RULES: Record<string, (evt: { source: SignalSource; title: string; body: string }) => SignalLevel> = {
  automation: (evt) => {
...
function classify(evt: { source: SignalSource; level?: SignalLevel; title: string; body: string }): SignalLevel {
