      ...
      >
        ...
        <div className="flex-1 overflow-y-auto scrollbar-mid">
          {Object.entries(grouped).map(([type, typeTasks]) => (
            <div key={type}>
              <div
                className="px-3 py-1.5 text-[9px] font-medium uppercase tracking-wider"
                style={{ color: 'var(--color-text-muted)' }}
              >
              ...
              {typeTasks.map((t) => (
                <button
                  ...
                  style={{
                    ...
                    background:
                      selectedTask?.id === t.id ? 'var(--color-accent-muted)' : 'transparent',
                  }}
                  onMouseEnter={(e) => {
                  ...
                  onMouseLeave={(e) => {
                    if (selectedTask?.id !== t.id) e.currentTarget.style.background = 'transparent';
                  }}
                >
                  ...
                  <div className="text-[10px] mt-0.5" style={{ color: 'var(--color-text-muted)' }}>
                    ...
                    {(t.schedule / 60000).toFixed(0)}min
                  </div>
                </button>
              ))}
            </div>
          ...
          {selectedTask ? (
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-[11px]" style={{ color: 'var(--color-text-muted)' }}>
                    {selectedTask.sourceUrl || 'No URL configured'}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => {
                      if (selectedTask.status === 'running') pauseTask(selectedTask.id);
                      else resumeTask(selectedTask.id);
                    }}
              ...
              <div className="space-y-3">
                ...
                >
                  <span
                    className="text-[10px] font-medium uppercase mb-2 block"
                    style={{ color: 'var(--color-text-muted)' }}
                  >
                    Configuration
                  </span>
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    ...
                    <div>
                      <span style={{ color: 'var(--color-text-muted)' }}>Interval:</span>{' '}
                      <span style={{ color: 'var(--color-text-secondary)' }}>
                        {Math.round(selectedTask.schedule / 60000).toString()} min
                      </span>
                    </div>
                    <div>
