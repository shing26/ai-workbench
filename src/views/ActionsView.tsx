...
const colorClass: Record<(typeof HABIT_COLORS)[number], { dot: string; active: string; ring: string }> = {
  emerald: {
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <BentoCard title="Habits" subtitle="打卡与连续天数" icon={Flame} colSpan={5}>
          ...
          <div className="flex flex-col gap-1.5">
                ...
                >
                  <button
                    ...
                    className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-lg border ${
                      h.doneToday ? "check-pop border-emerald-500/40 bg-emerald-500/20 text-emerald-400" : "border-white/20 text-transparent hover:border-white/40"
                    }`}
                  >
                    <Check size={12} />
                  </button>
                  <div className="min-w-0 flex-1">
                    <div className={`truncate text-xs ${h.doneToday ? "text-current" : "text-slate-300"}`}>{h.name}</div>
            ...
            {habits.length === 0 && <div className="py-8 text-center text-xs text-slate-600">No habits</div>}
          </div>
        </BentoCard>
        ...
        >
          ...
          <div className="flex flex-col gap-1.5">
            {scheduleEvents.map((ev) => (
              ...
              >
                <div className="w-10 shrink-0 text-right font-mono text-[10px] text-slate-500">{ev.startTime}</div>
                ...
                <button
                  ...
                  className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-lg border ${
                    ev.done ? "check-pop border-emerald-500/40 bg-emerald-500/20 text-emerald-400" : "border-white/20 text-transparent hover:border-white/40"
                  }`}
                >
                  <Check size={12} />
                </button>
                <span className={`min-w-0 flex-1 truncate text-xs ${ev.done ? "text-slate-600 line-through" : "text-slate-300"}`}>
                  {ev.title}
                </span>
                <span className="shrink-0 rounded-md bg-white/[0.06] px-2 py-0.5 text-[10px] text-slate-500">{ev.tag}</span>
              </div>
            ))}
            {scheduleEvents.length === 0 && <div className="py-8 text-center text-xs text-slate-600">No events</div>}
          </div>
        </BentoCard>
      </div>
    </div>
